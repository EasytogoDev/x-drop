    ...
    fetch(`${process.env.REACT_APP_API_ENDPOINT}/api/fornecedores/produtos/3`, requestOptions)
      ...
      .then((result) => {
        const mappedProducts = result.map(product => {
          // Filtrar a tabela "DROP" e obter o preço correspondente
          const tabelaDrop = product.VariacoesProduto?.[0]?.PrecosTabelas?.find(tabela => tabela.Tabela === "DROP");
          let priceDrop = tabelaDrop ? tabelaDrop.PrecoVenda : product.PrecoVenda;

          // Verificar se o valor é muito alto (indicando que está em centavos)
          if (priceDrop > 1000) {
            priceDrop = priceDrop / 100; // Ajustar o valor, se necessário
          }

          // Formatando o preço para o padrão Real
          ...
          return {
            ...
            price: priceFormatted, // Usar o preço formatado
